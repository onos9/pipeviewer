//! This Module contains the start loop
//!
//! # Some Header
//! More Discussion
...
pub fn stats_loop(silent: bool, rx: Receiver<usize>) -> Result<()> {
    let mut total_bytes = 0;
    let start = Instant::now();
    let mut timer = Timer::new();
    let mut sdterr = io::stderr();
    loop {
        let num_bytes = rx.recv().unwrap();
        timer.update();
...
fn output_progress(stderr: &mut Stderr, bytes: usize, elapse: String, rate: f64) {
    let bytes = style::style(format!("{} ", bytes)).with(Color::Red);
    let elapsed = style::style(elapse).with(Color::Green);
...
/// The TimeOutput trait adds a `.as_time` to `u64`
///
/// # Example
...
impl TimeOutput for u64 {
    /// Renders the u64 into a time string format
    fn as_time(&self) -> String {
